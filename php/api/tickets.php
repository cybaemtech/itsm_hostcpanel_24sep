...
function handleCreateTicket($request) {
    ...
    try {
        ...
        // Retrieve the created ticket with relations - using positional parameters for cPanel compatibility
        $ticket = $db->fetchOne("
            SELECT 
                t.*,
                c.name as category_name,
...
function handleGetSingleTicket($ticketId) {
    ...
    if ($userRole === 'user') {
        $sql = "
            ...
            LEFT JOIN users cb ON t.created_by_id = cb.id
            LEFT JOIN users at ON t.assigned_to_id = at.id
            WHERE t.id = ? AND (t.created_by_id = ? OR t.assigned_to_id = ?)";
        $params = [$ticketId, $userId, $userId];
    } else {
        $sql = "
            SELECT 
                t.*,
